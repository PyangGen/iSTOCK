@php
  $expiresAtRaw = session('admin_register_data.expires_at');
...
<head>
  ...
  <link rel="stylesheet" href="{{ asset('assets/css/admin/create/verify-email.css') }}">
</head>
  ...
  <section class="right">
    <div class="panel">
      ...
      <p class="subtitle">
        ...
        <strong>{{ session('admin_register_data.email') }}</strong>
      </p>
...
<p class="error-text client" id="clientCodeError" style="display:none;"></p>
      ...
      <form action="{{ route('admin.verify-email.check') }}"
      method="POST"
      class="form"
      id="verifyForm"
      data-expires-at="{{ $expiresAt }}">
        ...
        <div class="meta">
          <div class="left-meta">
            ...
            <a href="{{ route('admin.resend-code') }}" class="link">Resend now</a>
          </div>
